...
func (r *ClusterDownstreamWorkloadReconciler) Reconcile(ctx context.Context, req mcreconcile.Request) (_ ctrl.Result, err error) {
	...
	logger.Info("reconciling downstream workload")
	...
	if !downstreamWorkload.DeletionTimestamp.IsZero() {
		...
		return ctrl.Result{}, r.Finalize(ctx, downstreamClient, nil, nil, &downstreamWorkload)
	}
	...
	cl, err := r.mgr.GetCluster(ctx, downstreamWorkload.Spec.UpstreamWorkloadRef.ClusterName)
	if err != nil {
		return ctrl.Result{}, err
	}
	...
	workloadObjectKey := client.ObjectKey{
		Namespace: downstreamWorkload.Spec.UpstreamWorkloadRef.Namespace,
		Name:      downstreamWorkload.Spec.UpstreamWorkloadRef.Name,
	}
	if err := cl.GetClient().Get(ctx, workloadObjectKey, &workload); client.IgnoreNotFound(err) != nil {
		return ctrl.Result{}, fmt.Errorf("failed fetching upstream workload: %w", err)
	}

	if dt := workload.DeletionTimestamp; workload.CreationTimestamp.IsZero() || !dt.IsZero() {
		// Upstream workload is gone or deleting, run through the finalizer
		return ctrl.Result{}, r.Finalize(ctx, downstreamClient, nil, nil, &downstreamWorkload)
	}

	// TODO(jreese) add finalizer to upstream workload?

	downstreamStrategy := downstreamclient.NewMappedNamespaceResourceStrategy(
		downstreamWorkload.Spec.UpstreamWorkloadRef.ClusterName,
		cl.GetClient(),
		downstreamClient,
		r.Config.DownstreamResourceManagement.ManagedResourceLabels,
	)
	...
	result, err := r.awsWorkloadReconciler.Reconcile(
		ctx,
		downstreamStrategy,
		downstreamStrategy.GetClient(),
		downstreamWorkload.Spec.UpstreamWorkloadRef.ClusterName,
		&workload,
		&downstreamWorkload,
	)
...
func (r *ClusterDownstreamWorkloadReconciler) Finalize(
	ctx context.Context,
	...
	logger := log.FromContext(ctx)
	logger.Info("finalizing downstream workload")

	// if location.Spec.Provider.AWS != nil {
	result, err := r.awsWorkloadReconciler.Finalize(ctx, upstreamClient, r.DownstreamCluster, workload, downstreamWorkload)
	if err != nil {
	...
	// Finalize upstream workload at some point? Don't really want potentially
	// dozens of finalizers.
	// if controllerutil.RemoveFinalizer(downstreamWorkload, gcpInfraFinalizer) {
	// 	if err := upstreamClient.Update(ctx, downstreamWorkload); err != nil {
...
func (r *ClusterDownstreamWorkloadReconciler) SetupWithManager(mgr mcmanager.Manager) error {
	...
	return mcbuilder.ControllerManagedBy(mgr).
		Watches(&computev1alpha.Workload{}, func(clusterName string, cl cluster.Cluster) handler.TypedEventHandler[client.Object, mcreconcile.Request] {
			return handler.TypedEnqueueRequestsFromMapFunc(func(ctx context.Context, obj client.Object) []mcreconcile.Request {

				listOpts := client.MatchingFields{
					clusterDownstreamWorkloadIdentifierIndex: fmt.Sprintf(
						"%s/%s/%s",
						clusterName,
						obj.GetNamespace(),
						obj.GetName(),
				...
				var clusterDownstreamWorkloads infrav1alpha1.ClusterDownstreamWorkloadList
				if err := r.DownstreamCluster.GetClient().List(ctx, &clusterDownstreamWorkloads, listOpts); err != nil {
					log.FromContext(ctx).Info("failed listing downstream workloads: %w", err)
					return nil
				}

				if len(clusterDownstreamWorkloads.Items) == 0 {
					return nil
				}

				requests := make([]mcreconcile.Request, len(clusterDownstreamWorkloads.Items))
				for i, clusterDownstreamWorkload := range clusterDownstreamWorkloads.Items {
					requests[i] = mcreconcile.Request{
						...
						Request: reconcile.Request{
							NamespacedName: types.NamespacedName{
								Namespace: clusterDownstreamWorkload.Namespace,
								Name:      clusterDownstreamWorkload.Name,
							},
						},
					}
				}
		...
		Named("clusterdownstreamworkload").
		Complete(r)
}
